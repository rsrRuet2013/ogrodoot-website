...
export async function PATCH(request: Request, context: RouteContext<"/api/admin/users/[id]">) {
  ...
  try {
    ...
    if (body.status !== undefined) { if (!["pending", "approved", "rejected"].includes(String(body.status))) return NextResponse.json({ error: "Invalid status." }, { status: 400 }); update.status = String(body.status); }
    if (body.subTeam !== undefined) { if (!isSubTeam(String(body.subTeam))) return NextResponse.json({ error: "Invalid sub-team." }, { status: 400 }); update.subTeam = String(body.subTeam); }
    if (body.position !== undefined) { const value = String(body.position).trim(); if (value.length < 2 || value.length > 100) return NextResponse.json({ error: "Invalid position." }, { status: 400 }); update.position = value; }
    if (body.role !== undefined) { if (!["member", "lead", "admin"].includes(String(body.role))) return NextResponse.json({ error: "Invalid role." }, { status: 400 }); update.role = String(body.role); }
  ...
  } catch (error) { console.error("Member update failed", error); return NextResponse.json({ error: "Unable to update member." }, { status: 500 }); }
}
